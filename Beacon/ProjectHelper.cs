...
namespace Beacon
{
    public class ProjectHelper
    {
        public static void DoStuff()
        {
            ...
            void DoSpire()
            {
                SystemHelper.ExecuteApplication("xcopy", $"{Path.Combine(clientPath, @"src\FrontEnd\modules\blueprints")} {Path.Combine(commercePath, @"FrontEnd\modules\blueprints")} /E/Y", null, true);
                SystemHelper.ExecuteApplication("xcopy", $"{Path.Combine(clientPath, @"src\FrontEnd\modules\blueprints-shell")} {Path.Combine(commercePath, @"FrontEnd\modules\blueprints-shell")} /E/Y", null, true);
                SystemHelper.ExecuteApplication("cmd", "/c npm install", Path.Combine(commercePath, @"FrontEnd"), true);
            ...
            void DoClassic()
            {
                var webCsProj = Path.Combine(commercePath, "Legacy", "InsiteCommerce.Web", "InsiteCommerce.Web.csproj");
                var text = File.ReadAllLines(webCsProj).ToList();
                text.Insert(text.Count - 2, @$"   <ItemGroup>
    <Reference Include=""Extensions, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null"">
      <HintPath>..\..\..\clients\{clientName}\dist\Extensions.dll</HintPath>
                ...
                {
                    SystemHelper.ExecuteApplication("xcopy", $"{Path.Combine(clientPath, @"src\InsiteCommerce.Web\Themes")} {Path.Combine(commercePath, @"Legacy\InsiteCommerce.Web\Themes")} /E/Y", null, true);
                    // TODO grunt build themes
                }
                
                // TODO where do I find this?
                var result = SystemHelper.ExecuteApplication(@"C:\Program Files (x86)\Microsoft Visual Studio\2019\Professional\MSBuild\Current\Bin\msbuild.exe", @"C:\projects\insite-commerce\Legacy\InsiteCommerce.sln -m", null, true);
